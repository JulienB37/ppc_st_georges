...
export class SvgDraw {
  ...
  public async drawGroup(groupe: ConfigGroupe, x: number, y: number, width: number) {
    this._canvas
      .text(`${groupe.date} - ${groupe.domicile ? 'A domicile' : "A l'exterieur"}`)
      .font({
        family: 'Arial',
        size: 5,
        weight: 'bold',
  ...
  public async drawMatchEnfant(enfants: ConfigEnfant, x: number, y: number, width: number) {
    ...
    this._canvas
      .line([x + width / 2, y + 15, x + width / 2, y + 180])
      .stroke({ width: this._strokeWidthChild, color: 'black' });

    this._canvas
      .circle(this._circleDiameterChild)
      .fill('white')
      .stroke({
        color: 'red',
    ...
    this._canvas
      .rect(this._rectWidthChild, this._rectHeigthChild)
      .radius(2)
      .fill('white')
      .stroke({
        color: 'red',
      ...
      .move(
        x + width / 4 - this._rectWidthChild / 2,
        y + 20 + this._circleDiameterChild - this._rectHeigthChild / 1.5,
      );

    this._canvas
      .text(
        enfants.matche_1.domicile
          ? enfants.matche_1.equipe_st_georges
          : enfants.matche_1.equipe_adverse,
      )
      .font({
        family: 'Arial',
        size: 5,
      ...
      .center(
        ...
        y +
          20 +
          this._circleDiameterChild -
          this._rectHeigthChild / 1.5 +
          this._rectHeigthChild / 2,
      );
    ...
    this._canvas
      ...
      .move(
        x + width - width / 4 - this._rectWidthChild / 2,
        y + 20 + this._circleDiameterChild - this._rectHeigthChild / 1.5,
      );

    this._canvas
      .text(
        enfants.matche_2.domicile
          ? enfants.matche_2.equipe_st_georges
          : enfants.matche_2.equipe_adverse,
      )
      .font({
        family: 'Arial',
        size: 5,
    ...
    this._canvas
      ...
      .move(
        x + width / 4 - this._rectWidthChild / 2,
        y + 120 + this._circleDiameterChild - this._rectHeigthChild / 1.5,
      );
  ...
  private async drawMatches(
    matches: ConfigMatche[],
    domicile: boolean,
    x: number,
    y: number,
    width: number,
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      if (domicile) {
        ...
        this._canvas
          .text(match.equipe_st_georges)
          .font({
            family: 'Comic Sans MS',
            size: this._teamFontSize,
          })
          .move(
            x + this._teamHeigth + 4,
            y +
              (this._teamHeigth - (this._teamFontSize + this._teamGap * 2)) / 2 +
              (this._teamHeigth + this._teamGap) * i,
          );

        this._canvas
          .text(match.equipe_adverse)
          .font({
            family: 'Comic Sans MS',
            size: this._teamFontSize,
          })
          .move(
            x - 2 + (width - this._teamHeigth / 2) / 3 + this._teamHeigth,
            y +
              (this._teamHeigth - (this._teamFontSize + this._teamGap * 2)) / 2 +
              (this._teamHeigth + this._teamGap) * i,
          );
        ...
        this._canvas
          ...
          .move(
            x + (width - this._teamHeigth / 2) / 3 - this._teamHeigth / 4,
            y +
              (this._teamHeigth - (this._teamFontSize + this._teamGap * 2)) / 2 +
              (this._teamHeigth + this._teamGap) * i,
          );
        ...
        this._canvas
          ...
          .move(
            x + width - 2 - (width - this._teamHeigth / 2) / 3 + this._teamHeigth,
            y +
              (this._teamHeigth - (this._teamFontSize + this._teamGap * 2)) / 2 +
              (this._teamHeigth + this._teamGap) * i,
          );
        ...
        this._canvas
          ...
          .move(
            x + width - (width - this._teamHeigth / 2) / 3 - this._teamHeigth / 4,
            y +
              (this._teamHeigth - (this._teamFontSize + this._teamGap * 2)) / 2 +
              (this._teamHeigth + this._teamGap) * i,
          );
      ...
      if (domicile) {
        await this.drawImage(
          'PP St Georges/Cher',
          x + 3,
          y + 3 + (this._teamHeigth + this._teamGap) * i,
        );
        await this.drawImage(
          match.equipe_adverse,
          x + (width - this._teamHeigth) + 3,
          y + 3 + (this._teamHeigth + this._teamGap) * i,
        );
      } else {
        await this.drawImage(
  ...
  private async drawImage(equipe: string, x: number, y: number, size = this._teamHeigth - 6) {
    ...
    if (filePath) {
      ...
      this._canvas
        .image(`data:image/${fileext};base64,${base64}`)
        .size(size, size)
